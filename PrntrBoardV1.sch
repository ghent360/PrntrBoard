EESchema Schematic File Version 5
LIBS:PrntrBoardV1-cache
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 35
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 6050 950  1300 2550
U 5AC337B6
F0 "MotorControl" 50
F1 "MotorControl.sch" 50
F2 "X_STEP" I R 7350 1050 50 
F3 "X_DIR" I R 7350 1600 50 
F4 "X_SG" O L 6050 2850 50 
F5 "X_END_OUT" O R 7350 2700 50 
F6 "X_START_OUT" O R 7350 3200 50 
F7 "V_IO" I L 6050 1050 50 
F8 "V_MOTORS" I L 6050 1150 50 
F9 "Y_STEP" I R 7350 1150 50 
F10 "Y_DIR" I R 7350 1700 50 
F11 "Y_SG" O L 6050 2950 50 
F12 "Y_END_OUT" O R 7350 2800 50 
F13 "Y_START_OUT" O R 7350 3300 50 
F14 "~ENABLE" I L 6050 1500 50 
F15 "MISO" O L 6050 1900 50 
F16 "MOSI" I L 6050 1800 50 
F17 "SCK" I L 6050 1700 50 
F18 "Z_STEP" I R 7350 1250 50 
F19 "Z_DIR" I R 7350 1800 50 
F20 "Z_SG" O L 6050 3050 50 
F21 "Z_END_OUT" O R 7350 2900 50 
F22 "Z_START_OUT" O R 7350 3400 50 
F23 "AVREF" I L 6050 1250 50 
F24 "E0_STEP" I R 7350 1350 50 
F25 "E0_DIR" I R 7350 1900 50 
F26 "E0_SG" O L 6050 3150 50 
F27 "E0_FLMT" O R 7350 3000 50 
F28 "E0_TEMP" O L 6050 2250 50 
F29 "E0_PWM" I L 6050 2500 50 
F30 "E1_STEP" I R 7350 1450 50 
F31 "E1_DIR" I R 7350 2000 50 
F32 "E1_SG" O L 6050 3250 50 
F33 "E1_FLMT" O R 7350 3100 50 
F34 "E1_TEMP" O L 6050 2350 50 
F35 "E1_PWM" I L 6050 2600 50 
F36 "MOSI_BUF" O L 6050 2000 50 
F37 "SCK_BUF" O L 6050 2100 50 
F38 "UART_XYZ" B R 7350 2150 50 
F39 "UART_EX" B R 7350 2300 50 
F40 "V_HEATER" I L 6050 1340 50 
$EndSheet
$Comp
L my_4xxx:4078 U2
U 1 1 5AB8C4A5
P 5250 3200
F 0 "U2" H 5450 3650 50  0000 C CNN
F 1 "4078" H 5400 3750 50  0000 C CNN
F 2 "Package_SO:TSSOP-14_4.4x5mm_P0.65mm" H 5250 3200 50  0001 C CNN
F 3 "" H 5250 3200 50  0001 C CNN
	1    5250 3200
	-1   0    0    1   
$EndComp
Wire Wire Line
	6050 2850 5750 2850
Wire Wire Line
	6050 2950 5750 2950
Wire Wire Line
	6050 3050 5750 3050
Wire Wire Line
	6050 3150 5750 3150
Wire Wire Line
	6050 3250 5750 3250
Wire Wire Line
	5750 3350 5750 3450
Connection ~ 5750 3450
Wire Wire Line
	5750 3450 5750 3550
$Comp
L power:GND #PWR011
U 1 1 5AB8C5FB
P 5750 3800
F 0 "#PWR011" H 5750 3550 50  0001 C CNN
F 1 "GND" H 5755 3627 50  0000 C CNN
F 2 "" H 5750 3800 50  0001 C CNN
F 3 "" H 5750 3800 50  0001 C CNN
	1    5750 3800
	1    0    0    -1  
$EndComp
Connection ~ 5750 3550
Wire Wire Line
	5400 1050 6050 1050
$Comp
L Device:C C4
U 1 1 5AB8EAB0
P 5150 2300
F 0 "C4" H 4950 2350 50  0000 L CNN
F 1 "0.1uF" H 4850 2250 50  0000 L CNN
F 2 "PrntrBoardV1:C_0603_1608Metric" H 5188 2150 50  0001 C CNN
F 3 "~" H 5150 2300 50  0001 C CNN
	1    5150 2300
	1    0    0    -1  
$EndComp
Wire Wire Line
	5150 2150 5400 2150
Wire Wire Line
	5400 2150 5400 1250
$Comp
L power:GND #PWR08
U 1 1 5AB8EC73
P 5150 2500
F 0 "#PWR08" H 5150 2250 50  0001 C CNN
F 1 "GND" H 5050 2400 50  0000 C CNN
F 2 "" H 5150 2500 50  0001 C CNN
F 3 "" H 5150 2500 50  0001 C CNN
	1    5150 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	5150 2450 5150 2500
Text Notes 4250 4750 0    50   ~ 0
Pin budget:\n17 GPIO out\n7 GPIO PWM out\n12 GPIO in\n3 ADC in\n1 SPI
Wire Wire Line
	4750 3350 4650 3350
Text Label 4700 3350 0    50   ~ 0
STALL
$Sheet
S 6600 3800 1050 1050
U 5AB8F10D
F0 "Heated Bed" 50
F1 "BedHeater.sch" 50
F2 "AVREF" I L 6600 3950 50 
F3 "TEMP" O L 6600 4300 50 
F4 "PWM" I L 6600 4500 50 
F5 "V_HEAT" I R 7650 3950 50 
F6 "HEATER" O R 7650 4300 50 
$EndSheet
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J7
U 1 1 5AB8F2FF
P 8100 4150
F 0 "J7" H 8200 4200 50  0000 L CNN
F 1 "Bed Heater" H 7950 4300 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_MKDS-1,5-2-5.08_1x02_P5.08mm_Horizontal" H 8100 4150 50  0001 C CNN
F 3 "~" H 8100 4150 50  0001 C CNN
	1    8100 4150
	1    0    0    1   
$EndComp
Wire Wire Line
	7650 3950 7900 3950
Wire Wire Line
	7900 3950 7900 4050
Wire Wire Line
	7900 4150 7900 4300
Wire Wire Line
	7900 4300 7650 4300
Wire Wire Line
	6050 1150 5900 1150
Wire Wire Line
	5900 1150 5900 800 
Wire Wire Line
	5900 800  7900 800 
Wire Wire Line
	6050 1250 5950 1250
Wire Wire Line
	5950 1250 5950 3750
Wire Wire Line
	5950 3750 6450 3750
Wire Wire Line
	6450 3750 6450 3950
Wire Wire Line
	6450 3950 6600 3950
Text Label 2900 800  0    50   ~ 0
V_MOTORS
Text Label 5500 1050 0    50   ~ 0
3.3V
Text Label 6100 3750 0    50   ~ 0
AVREF
$Sheet
S 8300 1600 600  450 
U 5AB9002A
F0 "E0 Cooling Fan" 50
F1 "FanControl.sch" 50
F2 "OUT" O R 8900 1750 50 
F3 "PWM" I R 8900 1950 50 
F4 "V_FAN" I L 8300 1750 50 
$EndSheet
$Sheet
S 8300 2200 600  400 
U 5AB91A5D
F0 "E0 Part Fan" 50
F1 "FanControl.sch" 50
F2 "OUT" O R 8900 2300 50 
F3 "PWM" I R 8900 2500 50 
F4 "V_FAN" I L 8300 2300 50 
$EndSheet
$Sheet
S 8300 2750 600  450 
U 5AB91A68
F0 "E1 Cooling Fan" 50
F1 "FanControl.sch" 50
F2 "OUT" O R 8900 2900 50 
F3 "PWM" I R 8900 3100 50 
F4 "V_FAN" I L 8300 2900 50 
$EndSheet
$Sheet
S 8300 3350 600  400 
U 5AB91A71
F0 "E1 Part Fan" 50
F1 "FanControl.sch" 50
F2 "OUT" O R 8900 3450 50 
F3 "PWM" I R 8900 3650 50 
F4 "V_FAN" I L 8300 3500 50 
$EndSheet
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J12
U 1 1 5AB92C7F
P 9400 3350
F 0 "J12" H 9479 3342 50  0000 L CNN
F 1 "E1 Part" H 9479 3251 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 9400 3350 50  0001 C CNN
F 3 "~" H 9400 3350 50  0001 C CNN
	1    9400 3350
	1    0    0    -1  
$EndComp
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J11
U 1 1 5AB92D01
P 9400 2800
F 0 "J11" H 9479 2792 50  0000 L CNN
F 1 "E1 Cool" H 9479 2701 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 9400 2800 50  0001 C CNN
F 3 "~" H 9400 2800 50  0001 C CNN
	1    9400 2800
	1    0    0    -1  
$EndComp
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J10
U 1 1 5AB92D3C
P 9400 2200
F 0 "J10" H 9479 2192 50  0000 L CNN
F 1 "E0 Part" H 9479 2101 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 9400 2200 50  0001 C CNN
F 3 "~" H 9400 2200 50  0001 C CNN
	1    9400 2200
	1    0    0    -1  
$EndComp
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J9
U 1 1 5AB92D90
P 9400 1650
F 0 "J9" H 9479 1642 50  0000 L CNN
F 1 "E0 Cool" H 9479 1551 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 9400 1650 50  0001 C CNN
F 3 "~" H 9400 1650 50  0001 C CNN
	1    9400 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	8900 3450 9200 3450
Wire Wire Line
	8900 2900 9200 2900
Wire Wire Line
	8900 2300 9200 2300
Wire Wire Line
	8900 1750 9200 1750
Wire Wire Line
	9200 3350 9150 3350
Wire Wire Line
	9150 3350 9150 2800
Wire Wire Line
	9150 2800 9200 2800
Wire Wire Line
	9150 2800 9150 2200
Wire Wire Line
	9150 2200 9200 2200
Connection ~ 9150 2800
Wire Wire Line
	9150 2200 9150 1650
Wire Wire Line
	9150 1650 9200 1650
Connection ~ 9150 2200
Wire Wire Line
	9150 1650 9150 1250
Connection ~ 9150 1650
Wire Wire Line
	7900 800  7900 1050
Wire Wire Line
	8300 3500 8150 3500
Wire Wire Line
	8150 3500 8150 2900
Wire Wire Line
	8150 2900 8300 2900
Wire Wire Line
	8150 2900 8150 2300
Wire Wire Line
	8150 2300 8300 2300
Connection ~ 8150 2900
Wire Wire Line
	8150 2300 8150 1750
Wire Wire Line
	8150 1750 8300 1750
Connection ~ 8150 2300
Wire Wire Line
	8150 1750 8150 1250
Wire Wire Line
	8150 1250 8950 1250
Connection ~ 8150 1750
$Comp
L Jumper:Jumper_3_Open JP5
U 1 1 5AB991D4
P 8950 1050
F 0 "JP5" H 8950 1274 50  0000 C CNN
F 1 "FAN Voltage select" H 8950 1183 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 8950 1050 50  0001 C CNN
F 3 "~" H 8950 1050 50  0001 C CNN
	1    8950 1050
	1    0    0    -1  
$EndComp
Wire Wire Line
	8950 1200 8950 1250
Connection ~ 8950 1250
Wire Wire Line
	8950 1250 9150 1250
Wire Wire Line
	9200 1050 9500 1050
Text Label 9350 1050 0    50   ~ 0
5V
$Comp
L 74ls02:74HCT02 U3
U 1 1 5AB9C30F
P 5750 6750
F 0 "U3" H 5750 7075 50  0000 C CNN
F 1 "74HCT02" H 5750 6984 50  0000 C CNN
F 2 "Package_SO:TSSOP-14_4.4x5mm_P0.65mm" H 5750 6750 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74hct02" H 5750 6750 50  0001 C CNN
	1    5750 6750
	1    0    0    -1  
$EndComp
$Comp
L 74ls02:74HCT02 U3
U 2 1 5AB9E04D
P 5750 5200
F 0 "U3" H 5750 5525 50  0000 C CNN
F 1 "74HCT02" H 5750 5434 50  0000 C CNN
F 2 "Package_SO:TSSOP-14_4.4x5mm_P0.65mm" H 5750 5200 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74hct02" H 5750 5200 50  0001 C CNN
	2    5750 5200
	1    0    0    -1  
$EndComp
$Comp
L 74ls02:74HCT02 U3
U 3 1 5AB9E0C5
P 5750 5750
F 0 "U3" H 5750 6075 50  0000 C CNN
F 1 "74HCT02" H 5750 5984 50  0000 C CNN
F 2 "Package_SO:TSSOP-14_4.4x5mm_P0.65mm" H 5750 5750 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74hct02" H 5750 5750 50  0001 C CNN
	3    5750 5750
	1    0    0    -1  
$EndComp
$Comp
L 74ls02:74HCT02 U3
U 4 1 5AB9E15A
P 5750 6250
F 0 "U3" H 5750 6575 50  0000 C CNN
F 1 "74HCT02" H 5750 6484 50  0000 C CNN
F 2 "Package_SO:TSSOP-14_4.4x5mm_P0.65mm" H 5750 6250 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74hct02" H 5750 6250 50  0001 C CNN
	4    5750 6250
	1    0    0    -1  
$EndComp
$Comp
L 74ls02:74HCT02 U3
U 5 1 5AB9E1C8
P 7650 5600
F 0 "U3" H 7880 5646 50  0000 L CNN
F 1 "74HCT02" H 7880 5555 50  0000 L CNN
F 2 "Package_SO:TSSOP-14_4.4x5mm_P0.65mm" H 7650 5600 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74hct02" H 7650 5600 50  0001 C CNN
	5    7650 5600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5450 6150 5400 6150
Wire Wire Line
	5400 6150 5400 5650
Connection ~ 5400 5650
Wire Wire Line
	5400 5650 5450 5650
Wire Wire Line
	5400 5650 5400 5300
Wire Wire Line
	5400 5300 5450 5300
Wire Wire Line
	6050 5200 6100 5200
Wire Wire Line
	6100 5200 6100 4500
Wire Wire Line
	6050 5750 6400 5750
Wire Wire Line
	6050 6250 6400 6250
Text Label 6100 5750 0    50   ~ 0
E0_PWM
Text Label 6100 6250 0    50   ~ 0
E1_PWM
Wire Wire Line
	6050 2500 5600 2500
Wire Wire Line
	6050 2600 5600 2600
Text Label 5650 2500 0    50   ~ 0
E0_PWM
Text Label 5650 2600 0    50   ~ 0
E1_PWM
Wire Wire Line
	5450 5100 4900 5100
Wire Wire Line
	5450 5850 4850 5850
Wire Wire Line
	5450 6350 4850 6350
$Comp
L power:GND #PWR014
U 1 1 5ABADA71
P 7650 6200
F 0 "#PWR014" H 7650 5950 50  0001 C CNN
F 1 "GND" H 7655 6027 50  0000 C CNN
F 2 "" H 7650 6200 50  0001 C CNN
F 3 "" H 7650 6200 50  0001 C CNN
	1    7650 6200
	1    0    0    -1  
$EndComp
Wire Wire Line
	7650 6100 7650 6150
$Comp
L Device:C C7
U 1 1 5ABB120D
P 7150 5600
F 0 "C7" H 7265 5646 50  0000 L CNN
F 1 "0.1uF" H 7265 5555 50  0000 L CNN
F 2 "PrntrBoardV1:C_0603_1608Metric" H 7188 5450 50  0001 C CNN
F 3 "~" H 7150 5600 50  0001 C CNN
	1    7150 5600
	1    0    0    -1  
$EndComp
Wire Wire Line
	7150 5750 7150 6150
Wire Wire Line
	7150 6150 7650 6150
Connection ~ 7650 6150
Wire Wire Line
	7650 6150 7650 6200
Wire Wire Line
	7150 5450 7150 5350
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J13
U 1 1 5AC9DA10
P 10150 1650
F 0 "J13" H 10229 1642 50  0000 L CNN
F 1 "Always On Fan 1" H 9900 1450 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 10150 1650 50  0001 C CNN
F 3 "~" H 10150 1650 50  0001 C CNN
	1    10150 1650
	1    0    0    -1  
$EndComp
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J14
U 1 1 5AC9DAC8
P 10150 2050
F 0 "J14" H 10229 2042 50  0000 L CNN
F 1 "Always On Fan 2" H 9950 1850 50  0000 L CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 10150 2050 50  0001 C CNN
F 3 "~" H 10150 2050 50  0001 C CNN
	1    10150 2050
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR017
U 1 1 5AC9DB9D
P 9950 2350
F 0 "#PWR017" H 9950 2100 50  0001 C CNN
F 1 "GND" H 9955 2177 50  0000 C CNN
F 2 "" H 9950 2350 50  0001 C CNN
F 3 "" H 9950 2350 50  0001 C CNN
	1    9950 2350
	1    0    0    -1  
$EndComp
Wire Wire Line
	9950 2350 9950 2150
Wire Wire Line
	9950 1750 9900 1750
Wire Wire Line
	9900 1750 9900 2150
Wire Wire Line
	9900 2150 9950 2150
Connection ~ 9950 2150
Wire Wire Line
	9950 2050 9850 2050
Wire Wire Line
	9850 2050 9850 1650
Wire Wire Line
	9850 1650 9950 1650
Wire Wire Line
	9850 1650 9850 1250
Wire Wire Line
	9850 1250 9150 1250
Connection ~ 9850 1650
Connection ~ 9150 1250
Text Label 4950 5100 0    50   ~ 0
BED_PWM_IN
Text Label 4900 5850 0    50   ~ 0
E0_PWM_IN
Text Label 4900 6350 0    50   ~ 0
E1_PWM_IN
Wire Wire Line
	6050 1500 5500 1500
Wire Wire Line
	6050 1700 5600 1700
Wire Wire Line
	6050 1800 5600 1800
Wire Wire Line
	6050 1900 5600 1900
Wire Wire Line
	6600 4300 6050 4300
Text Label 6150 4300 0    50   ~ 0
BED_TEMP
Text Label 5650 2250 0    50   ~ 0
E0_TEMP
Text Label 5650 2350 0    50   ~ 0
E1_TEMP
Text Label 5500 1500 0    50   ~ 0
~EN_MOTORS
Text Label 5650 1700 0    50   ~ 0
SCK
Text Label 5650 1800 0    50   ~ 0
MOSI
Text Label 5650 1900 0    50   ~ 0
MISO
Connection ~ 5400 1250
Wire Wire Line
	5400 1250 5400 1050
Text Label 4000 1850 2    50   ~ 0
5V
Wire Wire Line
	5900 800  4050 800 
Connection ~ 5900 800 
$Comp
L power:GND #PWR06
U 1 1 5ADA2005
P 3850 2050
F 0 "#PWR06" H 3850 1800 50  0001 C CNN
F 1 "GND" H 3855 1877 50  0000 C CNN
F 2 "" H 3850 2050 50  0001 C CNN
F 3 "" H 3850 2050 50  0001 C CNN
	1    3850 2050
	1    0    0    -1  
$EndComp
Wire Wire Line
	8900 1950 9450 1950
Wire Wire Line
	8900 3100 9450 3100
Wire Wire Line
	8900 3650 9450 3650
Text Label 9450 3650 2    50   ~ 0
E1_PART_PWM
Text Label 9450 3100 2    50   ~ 0
E1_COOL_PWM
Wire Wire Line
	8900 2500 9450 2500
Text Label 9450 2500 2    50   ~ 0
E0_PART_PWM
Text Label 9450 1950 2    50   ~ 0
E0_COOL_PWM
Wire Wire Line
	7150 5100 7650 5100
Wire Wire Line
	7350 1050 7800 1050
Wire Wire Line
	7350 1150 7800 1150
Wire Wire Line
	7350 1250 7800 1250
Wire Wire Line
	7350 1350 7800 1350
Wire Wire Line
	7350 1450 7800 1450
Wire Wire Line
	7350 1600 7800 1600
Wire Wire Line
	7350 1700 7800 1700
Wire Wire Line
	7350 1800 7800 1800
Wire Wire Line
	7350 1900 7800 1900
Wire Wire Line
	7350 2000 7800 2000
Wire Wire Line
	7350 2700 7800 2700
Wire Wire Line
	7350 2800 7800 2800
Wire Wire Line
	7350 2900 7800 2900
Wire Wire Line
	7350 3000 7800 3000
Wire Wire Line
	7350 3100 7800 3100
Wire Wire Line
	7350 3200 7800 3200
Wire Wire Line
	7350 3300 7800 3300
Wire Wire Line
	7350 3400 7800 3400
Text Label 7800 1050 2    50   ~ 0
X_STEP
Text Label 7800 1150 2    50   ~ 0
Y_STEP
Text Label 7800 1250 2    50   ~ 0
Z_STEP
Text Label 7800 1350 2    50   ~ 0
E0_STEP
Text Label 7800 1450 2    50   ~ 0
E1_STEP
Text Label 7800 1600 2    50   ~ 0
X_DIR
Text Label 7800 1700 2    50   ~ 0
Y_DIR
Text Label 7800 1800 2    50   ~ 0
Z_DIR
Text Label 7800 1900 2    50   ~ 0
E0_DIR
Text Label 7800 2000 2    50   ~ 0
E1_DIR
Text Label 7800 2700 2    50   ~ 0
X_END
Text Label 7800 2800 2    50   ~ 0
Y_END
Text Label 7800 2900 2    50   ~ 0
Z_END
Text Label 7800 3000 2    50   ~ 0
E0_END
Text Label 7800 3100 2    50   ~ 0
E1_END
Text Label 7800 3200 2    50   ~ 0
X_START
Text Label 7800 3300 2    50   ~ 0
Y_START
Text Label 7800 3400 2    50   ~ 0
Z_START
Wire Wire Line
	5400 6150 5400 6650
Wire Wire Line
	5400 6650 5450 6650
Connection ~ 5400 6150
$Comp
L power:GND #PWR010
U 1 1 5AE30327
P 5400 6700
F 0 "#PWR010" H 5400 6450 50  0001 C CNN
F 1 "GND" H 5300 6700 50  0000 C CNN
F 2 "" H 5400 6700 50  0001 C CNN
F 3 "" H 5400 6700 50  0001 C CNN
	1    5400 6700
	1    0    0    -1  
$EndComp
Wire Wire Line
	5400 6700 5400 6650
Connection ~ 5400 6650
Wire Wire Line
	6050 6750 6750 6750
Text Label 6750 6750 2    50   ~ 0
Z_PROBE_MOD_BUF
Wire Wire Line
	5450 6850 4750 6850
Text Label 5350 6850 2    50   ~ 0
Z_PROBE_MOD
Wire Wire Line
	6100 4500 6600 4500
$Comp
L Connector_Generic:Conn_01x04 J8
U 1 1 5AE532AA
P 8550 5100
F 0 "J8" H 8470 4675 50  0000 C CNN
F 1 "Z Probe" H 8470 4766 50  0000 C CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-4POS" H 8550 5100 50  0001 C CNN
F 3 "~" H 8550 5100 50  0001 C CNN
	1    8550 5100
	-1   0    0    1   
$EndComp
Wire Wire Line
	8750 4900 9300 4900
Text Label 9300 4900 2    50   ~ 0
5V
Wire Wire Line
	8750 5000 9500 5000
Text Label 9500 5000 2    50   ~ 0
Z_PROBE_MOD_BUF
$Comp
L Device:R R8
U 1 1 5AE5F39C
P 9050 5200
F 0 "R8" V 9150 5150 50  0000 C CNN
F 1 "1k" V 9150 5300 50  0000 C CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 8980 5200 50  0001 C CNN
F 3 "~" H 9050 5200 50  0001 C CNN
	1    9050 5200
	0    1    1    0   
$EndComp
Wire Wire Line
	8750 5200 8900 5200
Wire Wire Line
	8750 5100 9450 5100
Wire Wire Line
	9450 5100 9450 5500
$Comp
L power:GND #PWR015
U 1 1 5AE6BACB
P 9450 5500
F 0 "#PWR015" H 9450 5250 50  0001 C CNN
F 1 "GND" H 9455 5327 50  0000 C CNN
F 2 "" H 9450 5500 50  0001 C CNN
F 3 "" H 9450 5500 50  0001 C CNN
	1    9450 5500
	1    0    0    -1  
$EndComp
Wire Wire Line
	9200 5200 10100 5200
Text Label 10050 5200 2    50   ~ 0
Z_PROBE_IN
NoConn ~ 4750 3200
$Comp
L NUCLEO-64:NUCLEO-64 U1
U 1 1 5ABA9B04
P 2500 3600
F 0 "U1" H 2500 4865 50  0000 C CNN
F 1 "NUCLEO-64" H 2500 4774 50  0000 C CNN
F 2 "PrntrBoardV1:Nucleo-64" H 2500 3600 50  0001 C CNN
F 3 "DOCUMENTATION" H 2500 3600 50  0001 C CNN
	1    2500 3600
	1    0    0    -1  
$EndComp
$Comp
L NUCLEO-64:NUCLEO-64 U1
U 2 1 5ABA9BEA
P 2500 6050
F 0 "U1" H 2500 7315 50  0000 C CNN
F 1 "NUCLEO-64" H 2500 7224 50  0000 C CNN
F 2 "PrntrBoardV1:Nucleo-64" H 2500 6050 50  0001 C CNN
F 3 "DOCUMENTATION" H 2500 6050 50  0001 C CNN
	2    2500 6050
	1    0    0    -1  
$EndComp
Wire Wire Line
	3250 5450 3950 5450
Text Label 3750 5450 0    50   ~ 0
VBUS
NoConn ~ 3250 3100
NoConn ~ 1750 3100
NoConn ~ 1750 3200
NoConn ~ 3250 3900
NoConn ~ 3250 6850
NoConn ~ 3250 6950
NoConn ~ 3250 5950
NoConn ~ 3250 5550
$Comp
L power:GND #PWR04
U 1 1 5ABF41E4
P 3400 7100
F 0 "#PWR04" H 3400 6850 50  0001 C CNN
F 1 "GND" H 3405 6927 50  0000 C CNN
F 2 "" H 3400 7100 50  0001 C CNN
F 3 "" H 3400 7100 50  0001 C CNN
	1    3400 7100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR01
U 1 1 5ABF4248
P 1700 7150
F 0 "#PWR01" H 1700 6900 50  0001 C CNN
F 1 "GND" H 1705 6977 50  0000 C CNN
F 2 "" H 1700 7150 50  0001 C CNN
F 3 "" H 1700 7150 50  0001 C CNN
	1    1700 7150
	1    0    0    -1  
$EndComp
Wire Wire Line
	1750 5550 1700 5550
Wire Wire Line
	1700 5550 1700 7150
Wire Wire Line
	3250 6050 3400 6050
Wire Wire Line
	3250 6650 3400 6650
Connection ~ 3400 6650
Wire Wire Line
	3400 6650 3400 7100
Wire Wire Line
	3250 3700 3400 3700
Connection ~ 3400 6050
Wire Wire Line
	3250 3600 3400 3600
Wire Wire Line
	3400 3600 3400 3700
Connection ~ 3400 3700
Wire Wire Line
	3250 3000 3400 3000
Wire Wire Line
	3400 3000 3400 3600
Connection ~ 3400 3600
Wire Wire Line
	1750 3600 1700 3600
Wire Wire Line
	1700 3600 1700 5550
Connection ~ 1700 5550
Wire Wire Line
	1750 5450 1000 5450
Text Label 1050 5450 0    50   ~ 0
AVREF
NoConn ~ 1750 4300
NoConn ~ 3250 3200
NoConn ~ 1750 3000
Wire Wire Line
	5750 3550 5750 3800
Wire Wire Line
	5250 2800 5250 2450
Wire Wire Line
	5250 2450 5150 2450
Connection ~ 5150 2450
Wire Wire Line
	5250 3600 5250 3900
Wire Wire Line
	5250 3900 4400 3900
Wire Wire Line
	4400 3900 4400 3400
Wire Wire Line
	4400 1250 5400 1250
Wire Wire Line
	3250 4500 3950 4500
Text Label 3600 4500 0    50   ~ 0
BED_TEMP
Wire Wire Line
	3250 4400 3950 4400
Text Label 3750 4400 0    50   ~ 0
MOSI
Wire Wire Line
	1750 4400 1000 4400
Text Label 1050 4400 0    50   ~ 0
MISO
Wire Wire Line
	1000 4500 1750 4500
Wire Wire Line
	3250 4000 3950 4000
Wire Wire Line
	3250 4100 3950 4100
Wire Wire Line
	3250 4200 3950 4200
Wire Wire Line
	3250 4300 3950 4300
Wire Wire Line
	3250 2800 3900 2800
Wire Wire Line
	3250 2700 3900 2700
Text Label 3650 4000 0    50   ~ 0
E0_TEMP
Text Label 3650 4100 0    50   ~ 0
E1_TEMP
NoConn ~ 1750 6850
NoConn ~ 1750 6950
NoConn ~ 1750 4100
NoConn ~ 1750 4200
NoConn ~ 3250 3300
NoConn ~ 3250 3500
NoConn ~ 1750 2900
Wire Wire Line
	1750 3800 1000 3800
Wire Wire Line
	1750 3700 1000 3700
Wire Wire Line
	1750 3500 1000 3500
Wire Wire Line
	1750 2800 1000 2800
Wire Wire Line
	1750 2700 1000 2700
Text Label 3700 4200 0    50   ~ 0
X_STEP
Text Label 3500 4300 0    50   ~ 0
BED_PWM_IN
Text Label 3650 2800 0    50   ~ 0
X_END
Text Label 3600 2700 0    50   ~ 0
Y_START
Text Label 1050 2700 0    50   ~ 0
X_START
Text Label 1050 2800 0    50   ~ 0
Z_START
Text Label 1050 3500 0    50   ~ 0
STALL
Text Label 1050 3700 0    50   ~ 0
E1_END
Text Label 1050 3800 0    50   ~ 0
Z_PROBE_IN
Wire Wire Line
	3250 5150 3950 5150
Wire Wire Line
	3250 5350 3950 5350
Wire Wire Line
	3250 5850 3950 5850
Wire Wire Line
	3250 6350 3950 6350
Wire Wire Line
	3250 6550 3950 6550
Wire Wire Line
	3250 6750 3950 6750
Wire Wire Line
	1750 5150 1000 5150
Wire Wire Line
	1750 5250 1000 5250
Wire Wire Line
	1750 5350 1000 5350
Wire Wire Line
	1750 5650 1000 5650
Wire Wire Line
	1750 5750 1000 5750
Wire Wire Line
	1750 5850 1000 5850
Wire Wire Line
	1750 5950 1000 5950
Wire Wire Line
	1750 6050 1000 6050
Wire Wire Line
	1750 6150 1000 6150
Wire Wire Line
	1750 6350 1000 6350
Wire Wire Line
	1750 6450 1000 6450
Text Label 1050 5150 0    50   ~ 0
E1_STEP
Text Label 3650 5150 0    50   ~ 0
E0_DIR
Text Label 3650 5350 0    50   ~ 0
Y_STEP
Text Label 1175 7675 0    50   ~ 0
E0_COOL_PWM
Text Label 1050 5650 0    50   ~ 0
~EN_MOTORS
Text Label 1050 5750 0    50   ~ 0
E0_PWM_IN
Text Label 1050 5850 0    50   ~ 0
E1_PWM_IN
Text Label 1050 5950 0    50   ~ 0
E0_END
Text Label 1050 6050 0    50   ~ 0
E0_STEP
Text Label 1050 6150 0    50   ~ 0
E1_DIR
Text Label 1050 6350 0    50   ~ 0
SCK
Text Label 1050 6450 0    50   ~ 0
Y_END
Text Label 3750 5850 0    50   ~ 0
Y_DIR
Text Label 3450 6150 0    50   ~ 0
E0_PART_PWM
Text Label 3700 6350 0    50   ~ 0
Z_DIR
Text Label 3450 6450 0    50   ~ 0
E1_PART_PWM
Text Label 3650 6550 0    50   ~ 0
Z_STEP
Text Label 3700 6750 0    50   ~ 0
X_DIR
Wire Wire Line
	3250 2900 4100 2900
Wire Wire Line
	4100 2900 4100 4150
Wire Wire Line
	4100 4150 4950 4150
Wire Wire Line
	5850 4150 5850 4700
Wire Wire Line
	5850 4700 6400 4700
Wire Wire Line
	6400 4700 6400 5100
Wire Wire Line
	6400 5100 6500 5100
Connection ~ 7150 5100
Wire Wire Line
	3400 6050 3400 6650
Wire Wire Line
	3250 6150 3950 6150
Wire Wire Line
	3250 6450 3950 6450
Wire Wire Line
	3400 3700 3400 6050
Wire Wire Line
	3250 3400 3650 3400
Connection ~ 4400 3400
Wire Wire Line
	4400 3400 4400 2750
Connection ~ 4100 2900
$Comp
L power:GND #PWR02
U 1 1 5ACB9195
P 1850 950
F 0 "#PWR02" H 1850 700 50  0001 C CNN
F 1 "GND" H 1855 777 50  0000 C CNN
F 2 "" H 1850 950 50  0001 C CNN
F 3 "" H 1850 950 50  0001 C CNN
	1    1850 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	3700 1850 4100 1850
Wire Wire Line
	4100 1850 4100 2500
Wire Wire Line
	3850 2050 3850 1950
Wire Wire Line
	3850 1950 3700 1950
$Comp
L Mechanical:MountingHole MK2
U 1 1 5ACC3347
P 1000 1600
F 0 "MK2" H 1100 1646 50  0000 L CNN
F 1 "Mounting_Hole_3mm" H 1100 1555 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.2mm_M3" H 1000 1600 50  0001 C CNN
F 3 "" H 1000 1600 50  0001 C CNN
	1    1000 1600
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole MK3
U 1 1 5ACC33D5
P 1000 1850
F 0 "MK3" H 1100 1896 50  0000 L CNN
F 1 "Mounting_Hole_3mm" H 1100 1805 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.2mm_M3" H 1000 1850 50  0001 C CNN
F 3 "" H 1000 1850 50  0001 C CNN
	1    1000 1850
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole MK4
U 1 1 5ACC3448
P 1000 2100
F 0 "MK4" H 1100 2146 50  0000 L CNN
F 1 "Mounting_Hole_3mm" H 1100 2055 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.2mm_M3" H 1000 2100 50  0001 C CNN
F 3 "" H 1000 2100 50  0001 C CNN
	1    1000 2100
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole FID2
U 1 1 5ACF4B85
P 1950 1600
F 0 "FID2" H 2050 1646 50  0000 L CNN
F 1 "Fiducial" H 2050 1555 50  0000 L CNN
F 2 "PrntrBoardV1:Fiducial_1mm_Dia_2.54mm_Outer_CopperTop" H 1950 1600 50  0001 C CNN
F 3 "" H 1950 1600 50  0001 C CNN
	1    1950 1600
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole FID3
U 1 1 5ACF4C64
P 1950 1850
F 0 "FID3" H 2050 1896 50  0000 L CNN
F 1 "Fiducial" H 2050 1805 50  0000 L CNN
F 2 "PrntrBoardV1:Fiducial_1mm_Dia_2.54mm_Outer_CopperTop" H 1950 1850 50  0001 C CNN
F 3 "" H 1950 1850 50  0001 C CNN
	1    1950 1850
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole FID4
U 1 1 5ACF4CE0
P 1950 2100
F 0 "FID4" H 2050 2146 50  0000 L CNN
F 1 "Fiducial" H 2050 2055 50  0000 L CNN
F 2 "PrntrBoardV1:Fiducial_1mm_Dia_2.54mm_Outer_CopperTop" H 1950 2100 50  0001 C CNN
F 3 "" H 1950 2100 50  0001 C CNN
	1    1950 2100
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole FID1
U 1 1 5AD38017
P 1950 1400
F 0 "FID1" H 2050 1446 50  0000 L CNN
F 1 "Fiducial" H 2050 1355 50  0000 L CNN
F 2 "PrntrBoardV1:Fiducial_1mm_Dia_2.54mm_Outer_CopperTop" H 1950 1400 50  0001 C CNN
F 3 "" H 1950 1400 50  0001 C CNN
	1    1950 1400
	1    0    0    -1  
$EndComp
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J2
U 1 1 5AEDA4A7
P 3500 1950
F 0 "J2" H 3420 1625 50  0000 C CNN
F 1 "EXT 5V" H 3420 1716 50  0000 C CNN
F 2 "PrntrBoardV1:TerminalBlock_Phoenix_PT-1,5-2-3.5-H_1x02_P3.50mm_Horizontal" H 3500 1950 50  0001 C CNN
F 3 "~" H 3500 1950 50  0001 C CNN
	1    3500 1950
	-1   0    0    1   
$EndComp
Wire Wire Line
	4100 1350 4100 1850
Connection ~ 4100 1850
Wire Wire Line
	7900 1050 8700 1050
$Comp
L Device:CP C2
U 1 1 5ADECB62
P 4700 1500
F 0 "C2" H 4450 1550 50  0000 L CNN
F 1 "100uF" H 4450 1400 50  0000 L CNN
F 2 "Capacitor_SMD:CP_Elec_6.3x7.7" H 4738 1350 50  0001 C CNN
F 3 "~" H 4700 1500 50  0001 C CNN
	1    4700 1500
	1    0    0    -1  
$EndComp
Connection ~ 4700 1350
Wire Wire Line
	4700 1350 4100 1350
$Comp
L Device:C C3
U 1 1 5ADECC47
P 5050 1500
F 0 "C3" H 5165 1546 50  0000 L CNN
F 1 "1000pF" H 5050 1400 50  0000 L CNN
F 2 "PrntrBoardV1:C_0603_1608Metric" H 5088 1350 50  0001 C CNN
F 3 "~" H 5050 1500 50  0001 C CNN
	1    5050 1500
	1    0    0    -1  
$EndComp
Wire Wire Line
	5050 1350 4700 1350
Wire Wire Line
	4700 1950 4700 1650
Connection ~ 3850 1950
Wire Wire Line
	5050 1650 5050 1950
Wire Wire Line
	5050 1950 4700 1950
Connection ~ 4700 1950
$Comp
L Mechanical:MountingHole MK1
U 1 1 5AE11BDF
P 1000 1350
F 0 "MK1" H 1100 1396 50  0000 L CNN
F 1 "Mounting_Hole" H 1100 1305 50  0000 L CNN
F 2 "MountingHole:MountingHole_3.2mm_M3" H 1000 1350 50  0001 C CNN
F 3 "" H 1000 1350 50  0001 C CNN
	1    1000 1350
	1    0    0    -1  
$EndComp
$Comp
L Jumper:Jumper_2_Open JP2
U 1 1 5AF4F08D
P 3350 1350
F 0 "JP2" V 3304 1448 50  0000 L CNN
F 1 "Use VIn" V 3395 1448 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x02_P2.54mm_Vertical" H 3350 1350 50  0001 C CNN
F 3 "~" H 3350 1350 50  0001 C CNN
	1    3350 1350
	0    1    1    0   
$EndComp
Wire Wire Line
	3350 1150 3350 800 
Connection ~ 3350 800 
Wire Wire Line
	3350 800  3000 800 
Wire Wire Line
	3350 1550 3350 3800
Wire Wire Line
	3350 3800 3250 3800
$Comp
L Jumper:Jumper_2_Open JP3
U 1 1 5AF74BAA
P 3900 2500
F 0 "JP3" H 4200 2500 50  0000 C CNN
F 1 "Use USB" H 3900 2644 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x02_P2.54mm_Vertical" H 3900 2500 50  0001 C CNN
F 3 "~" H 3900 2500 50  0001 C CNN
	1    3900 2500
	1    0    0    -1  
$EndComp
Connection ~ 4100 2500
Wire Wire Line
	4100 2500 4100 2900
Wire Wire Line
	3700 2500 3450 2500
Text Label 3500 2500 0    50   ~ 0
VBUS
$Comp
L Device:R R6
U 1 1 5AFAB4C8
P 4900 4900
F 0 "R6" H 4970 4946 50  0000 L CNN
F 1 "10k" H 4970 4855 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 4830 4900 50  0001 C CNN
F 3 "~" H 4900 4900 50  0001 C CNN
	1    4900 4900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R4
U 1 1 5AFAB5E3
P 4850 5600
F 0 "R4" H 4920 5646 50  0000 L CNN
F 1 "10k" H 4920 5555 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 4780 5600 50  0001 C CNN
F 3 "~" H 4850 5600 50  0001 C CNN
	1    4850 5600
	1    0    0    -1  
$EndComp
$Comp
L Device:R R5
U 1 1 5AFAB683
P 4850 6100
F 0 "R5" H 4920 6146 50  0000 L CNN
F 1 "10k" H 4920 6055 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 4780 6100 50  0001 C CNN
F 3 "~" H 4850 6100 50  0001 C CNN
	1    4850 6100
	1    0    0    -1  
$EndComp
$Comp
L Device:R R3
U 1 1 5AFAB73E
P 4750 6650
F 0 "R3" H 4820 6696 50  0000 L CNN
F 1 "10k" H 4820 6605 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 4680 6650 50  0001 C CNN
F 3 "~" H 4750 6650 50  0001 C CNN
	1    4750 6650
	1    0    0    -1  
$EndComp
Wire Wire Line
	4750 6800 4750 6850
Wire Wire Line
	4850 6250 4850 6350
Wire Wire Line
	4850 5750 4850 5850
Wire Wire Line
	4900 5050 4900 5100
Wire Wire Line
	4750 6500 4750 6350
Wire Wire Line
	4850 5950 5050 5950
Wire Wire Line
	4850 5450 4850 5250
Wire Wire Line
	4900 4750 4900 4550
Text Label 4900 4650 0    50   ~ 0
3.3V
Text Label 4850 5350 0    50   ~ 0
3.3V
Text Label 4850 5950 0    50   ~ 0
3.3V
Text Label 4750 6450 0    50   ~ 0
3.3V
$Comp
L Device:LED D1
U 1 1 5AF9C2B5
P 3000 950
F 0 "D1" V 3038 833 50  0000 R CNN
F 1 "LED" V 2947 833 50  0000 R CNN
F 2 "PrntrBoardV1:LED_0603_1608Metric" H 3000 950 50  0001 C CNN
F 3 "~" H 3000 950 50  0001 C CNN
	1    3000 950 
	0    -1   -1   0   
$EndComp
$Comp
L Device:LED D4
U 1 1 5AF9C4CA
P 10750 1400
F 0 "D4" V 10788 1283 50  0000 R CNN
F 1 "LED" V 10697 1283 50  0000 R CNN
F 2 "PrntrBoardV1:LED_0603_1608Metric" H 10750 1400 50  0001 C CNN
F 3 "~" H 10750 1400 50  0001 C CNN
	1    10750 1400
	0    -1   -1   0   
$EndComp
$Comp
L Device:LED D3
U 1 1 5AF9C705
P 6500 5250
F 0 "D3" V 6538 5133 50  0000 R CNN
F 1 "LED" V 6447 5133 50  0000 R CNN
F 2 "PrntrBoardV1:LED_0603_1608Metric" H 6500 5250 50  0001 C CNN
F 3 "~" H 6500 5250 50  0001 C CNN
	1    6500 5250
	0    -1   -1   0   
$EndComp
$Comp
L Device:R R7
U 1 1 5AF9C901
P 6500 5600
F 0 "R7" H 6570 5646 50  0000 L CNN
F 1 "10k" H 6570 5555 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 6430 5600 50  0001 C CNN
F 3 "~" H 6500 5600 50  0001 C CNN
	1    6500 5600
	1    0    0    -1  
$EndComp
$Comp
L Device:R R1
U 1 1 5AF9C9AB
P 3000 1300
F 0 "R1" H 3070 1346 50  0000 L CNN
F 1 "10k" H 3070 1255 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 2930 1300 50  0001 C CNN
F 3 "~" H 3000 1300 50  0001 C CNN
	1    3000 1300
	1    0    0    -1  
$EndComp
$Comp
L Device:R R9
U 1 1 5AF9CA87
P 10750 1750
F 0 "R9" H 10820 1796 50  0000 L CNN
F 1 "10k" H 10820 1705 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 10680 1750 50  0001 C CNN
F 3 "~" H 10750 1750 50  0001 C CNN
	1    10750 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	9850 1250 10750 1250
Connection ~ 9850 1250
Wire Wire Line
	10750 1550 10750 1600
Wire Wire Line
	3000 1100 3000 1150
Wire Wire Line
	6500 5400 6500 5450
$Comp
L power:GND #PWR013
U 1 1 5AFEB104
P 6500 5800
F 0 "#PWR013" H 6500 5550 50  0001 C CNN
F 1 "GND" H 6505 5627 50  0000 C CNN
F 2 "" H 6500 5800 50  0001 C CNN
F 3 "" H 6500 5800 50  0001 C CNN
	1    6500 5800
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR03
U 1 1 5AFEB1AA
P 3000 1500
F 0 "#PWR03" H 3000 1250 50  0001 C CNN
F 1 "GND" H 3005 1327 50  0000 C CNN
F 2 "" H 3000 1500 50  0001 C CNN
F 3 "" H 3000 1500 50  0001 C CNN
	1    3000 1500
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR018
U 1 1 5AFEB250
P 10750 1950
F 0 "#PWR018" H 10750 1700 50  0001 C CNN
F 1 "GND" H 10755 1777 50  0000 C CNN
F 2 "" H 10750 1950 50  0001 C CNN
F 3 "" H 10750 1950 50  0001 C CNN
	1    10750 1950
	1    0    0    -1  
$EndComp
Wire Wire Line
	10750 1900 10750 1950
Wire Wire Line
	6500 5750 6500 5800
Wire Wire Line
	3000 1450 3000 1500
Wire Wire Line
	3850 1950 4700 1950
$Comp
L Device:R R2
U 1 1 5B04FD2D
P 4700 2350
F 0 "R2" H 4500 2450 50  0000 L CNN
F 1 "1k" H 4550 2350 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 4630 2350 50  0001 C CNN
F 3 "~" H 4700 2350 50  0001 C CNN
	1    4700 2350
	1    0    0    -1  
$EndComp
$Comp
L Device:LED D2
U 1 1 5B04FE03
P 4550 2750
F 0 "D2" H 4542 2495 50  0000 C CNN
F 1 "LED" H 4542 2586 50  0000 C CNN
F 2 "PrntrBoardV1:LED_0603_1608Metric" H 4550 2750 50  0001 C CNN
F 3 "~" H 4550 2750 50  0001 C CNN
	1    4550 2750
	-1   0    0    1   
$EndComp
Connection ~ 4400 2750
Wire Wire Line
	4400 2750 4400 1250
Wire Wire Line
	4700 2750 4700 2500
Wire Wire Line
	4700 2200 4700 1950
$Sheet
S 10000 3650 950  1200
U 5AFA5DB1
F0 "LCD Display" 50
F1 "ST7735 LCD.sch" 50
F2 "SDA" I L 10000 3800 50 
F3 "SCL" I L 10000 3900 50 
F4 "A0" I L 10000 4000 50 
F5 "~RESET" I L 10000 4100 50 
F6 "LED+" I L 10000 4700 50 
F7 "~CS" I L 10000 4200 50 
F8 "VCC" I L 10000 4300 50 
F9 "BTN_IN" I L 10000 4400 50 
F10 "ENC_A" I L 10000 4500 50 
F11 "ENC_B" I L 10000 4600 50 
F12 "MISO" O L 10000 4800 50 
$EndSheet
Text Label 9450 4700 0    50   ~ 0
3.3V
Wire Wire Line
	10000 3800 9400 3800
Wire Wire Line
	10000 3900 9400 3900
Wire Wire Line
	10000 4000 9400 4000
Wire Wire Line
	10000 4100 9400 4100
Text Label 9450 4100 0    50   ~ 0
~LCD_RESET
Text Label 9450 4000 0    50   ~ 0
LCD_A0
Text Label 9450 3900 0    50   ~ 0
SCK_BUF
Text Label 9450 3800 0    50   ~ 0
MOSI_BUF
Wire Wire Line
	1750 3900 1000 3900
Wire Wire Line
	10000 4200 9400 4200
Text Label 9450 4200 0    50   ~ 0
~LCD_CS
Wire Wire Line
	1750 4000 1000 4000
Wire Wire Line
	6050 2350 5600 2350
Wire Wire Line
	6050 2250 5600 2250
Wire Wire Line
	6050 2000 5600 2000
Wire Wire Line
	6050 2100 5600 2100
Text Label 5650 2000 0    50   ~ 0
MOSI_BUF
Text Label 5650 2100 0    50   ~ 0
SCK_BUF
Wire Wire Line
	9400 4700 10000 4700
Wire Wire Line
	10000 4300 9400 4300
Text Label 9450 4300 0    50   ~ 0
5V
$Comp
L Connector_Generic:Conn_01x06 J15
U 1 1 5B0DCBB0
P 10400 6000
F 0 "J15" H 10450 5950 50  0000 L CNN
F 1 "TC1" H 10350 6550 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-6POS" H 10400 6000 50  0001 C CNN
F 3 "~" H 10400 6000 50  0001 C CNN
	1    10400 6000
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x06 J16
U 1 1 5B0DCCBB
P 10900 6000
F 0 "J16" H 10950 5950 50  0000 L CNN
F 1 "TC2" H 10850 6550 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-6POS" H 10900 6000 50  0001 C CNN
F 3 "~" H 10900 6000 50  0001 C CNN
	1    10900 6000
	1    0    0    -1  
$EndComp
Wire Wire Line
	10700 6000 10600 6000
Wire Wire Line
	10600 6000 10600 5700
Wire Wire Line
	10600 5700 10100 5700
Wire Wire Line
	10100 5700 10100 6000
Wire Wire Line
	10100 6000 10200 6000
Wire Wire Line
	10700 5900 10650 5900
Wire Wire Line
	10650 5900 10650 5650
Wire Wire Line
	10650 5650 10150 5650
Wire Wire Line
	10150 5650 10150 5900
Wire Wire Line
	10150 5900 10200 5900
Wire Wire Line
	10700 5800 10700 5600
Wire Wire Line
	10700 5600 10200 5600
Wire Wire Line
	10200 5600 10200 5800
Wire Wire Line
	10700 6100 10600 6100
Wire Wire Line
	10600 6100 10600 6400
Wire Wire Line
	10600 6400 10100 6400
Wire Wire Line
	10100 6400 10100 6100
Wire Wire Line
	10100 6100 10200 6100
Wire Wire Line
	10700 6200 10650 6200
Wire Wire Line
	10650 6200 10650 6450
Wire Wire Line
	10650 6450 10150 6450
Wire Wire Line
	10150 6450 10150 6200
Wire Wire Line
	10150 6200 10200 6200
Wire Wire Line
	10700 6300 10700 6500
Text Label 10350 5600 0    50   ~ 0
5V
$Comp
L power:GND #PWR019
U 1 1 5B1853F1
P 11050 5650
F 0 "#PWR019" H 11050 5400 50  0001 C CNN
F 1 "GND" H 11055 5477 50  0000 C CNN
F 2 "" H 11050 5650 50  0001 C CNN
F 3 "" H 11050 5650 50  0001 C CNN
	1    11050 5650
	1    0    0    -1  
$EndComp
Wire Wire Line
	10650 5650 11050 5650
Connection ~ 10650 5650
Wire Wire Line
	10100 6000 9650 6000
Connection ~ 10100 6000
Wire Wire Line
	10100 6100 9650 6100
Connection ~ 10100 6100
Wire Wire Line
	10150 6200 9650 6200
Connection ~ 10150 6200
Wire Wire Line
	10200 6300 9650 6300
Wire Wire Line
	9650 6500 10700 6500
Text Label 9650 6000 0    50   ~ 0
SCK_BUF
Text Label 9650 6100 0    50   ~ 0
MOSI_BUF
Text Label 9650 6200 0    50   ~ 0
MISO
Text Label 9650 6300 0    50   ~ 0
~TC1_CS
Text Label 9650 6500 0    50   ~ 0
~TC2_CS
$Comp
L Jumper:Jumper_3_Bridged12 JP1
U 1 1 5B2464D1
P 750 6250
F 0 "JP1" V 796 6316 50  0000 L CNN
F 1 "E1_TC2" H 900 6450 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 750 6250 50  0001 C CNN
F 3 "~" H 750 6250 50  0001 C CNN
	1    750  6250
	0    -1   -1   0   
$EndComp
Wire Wire Line
	900  6250 1750 6250
Wire Wire Line
	750  6000 750  5650
Text Label 750  6000 1    50   ~ 0
~TC2_CS
Text Label 1000 6250 0    50   ~ 0
E1_COOL_TC2
$Comp
L Device:C C1
U 1 1 5B2D209E
P 3650 3550
F 0 "C1" H 3765 3596 50  0000 L CNN
F 1 "10uF 10V" H 3765 3505 50  0000 L CNN
F 2 "PrntrBoardV1:C_0603_1608Metric" H 3688 3400 50  0001 C CNN
F 3 "~" H 3650 3550 50  0001 C CNN
	1    3650 3550
	1    0    0    -1  
$EndComp
Connection ~ 3650 3400
Wire Wire Line
	3650 3400 4400 3400
$Comp
L Device:C C5
U 1 1 5B2D2211
P 5950 3900
F 0 "C5" H 6065 3946 50  0000 L CNN
F 1 "10uF 10V" H 6065 3855 50  0000 L CNN
F 2 "PrntrBoardV1:C_0603_1608Metric" H 5988 3750 50  0001 C CNN
F 3 "~" H 5950 3900 50  0001 C CNN
	1    5950 3900
	1    0    0    -1  
$EndComp
Connection ~ 5950 3750
$Comp
L power:GND #PWR05
U 1 1 5B2D230A
P 3650 3700
F 0 "#PWR05" H 3650 3450 50  0001 C CNN
F 1 "GND" H 3655 3527 50  0000 C CNN
F 2 "" H 3650 3700 50  0001 C CNN
F 3 "" H 3650 3700 50  0001 C CNN
	1    3650 3700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR012
U 1 1 5B2D23C8
P 5950 4050
F 0 "#PWR012" H 5950 3800 50  0001 C CNN
F 1 "GND" H 5955 3877 50  0000 C CNN
F 2 "" H 5950 4050 50  0001 C CNN
F 3 "" H 5950 4050 50  0001 C CNN
	1    5950 4050
	1    0    0    -1  
$EndComp
Connection ~ 6500 5100
Wire Wire Line
	6500 5100 7150 5100
$Comp
L Device:C C6
U 1 1 5B36C419
P 6900 5600
F 0 "C6" H 6950 5500 50  0000 L CNN
F 1 "1000pF" H 6950 5400 50  0000 L CNN
F 2 "PrntrBoardV1:C_0603_1608Metric" H 6938 5450 50  0001 C CNN
F 3 "~" H 6900 5600 50  0001 C CNN
	1    6900 5600
	1    0    0    -1  
$EndComp
Wire Wire Line
	6900 5450 6900 5350
Wire Wire Line
	6900 5350 7150 5350
Connection ~ 7150 5350
Wire Wire Line
	7150 5350 7150 5100
Wire Wire Line
	6900 5750 6900 6150
Wire Wire Line
	6900 6150 7150 6150
Connection ~ 7150 6150
$Comp
L Connector_Generic:Conn_01x02 J3
U 1 1 5B3CD5DC
P 3800 950
F 0 "J3" H 3879 942 50  0000 L CNN
F 1 "Vin" H 3879 851 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-2POS" H 3800 950 50  0001 C CNN
F 3 "~" H 3800 950 50  0001 C CNN
	1    3800 950 
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x02 J4
U 1 1 5B3CD7CC
P 4250 950
F 0 "J4" H 4329 942 50  0000 L CNN
F 1 "Vin" H 4329 851 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-2POS" H 4250 950 50  0001 C CNN
F 3 "~" H 4250 950 50  0001 C CNN
	1    4250 950 
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR07
U 1 1 5B3CD97B
P 3900 1200
F 0 "#PWR07" H 3900 950 50  0001 C CNN
F 1 "GND" H 3905 1027 50  0000 C CNN
F 2 "" H 3900 1200 50  0001 C CNN
F 3 "" H 3900 1200 50  0001 C CNN
	1    3900 1200
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 1050 3600 1200
Wire Wire Line
	3600 1200 3900 1200
Wire Wire Line
	4050 1050 4050 1200
Wire Wire Line
	4050 1200 3900 1200
Connection ~ 3900 1200
Wire Wire Line
	4050 950  4050 800 
Connection ~ 4050 800 
Wire Wire Line
	4050 800  3600 800 
Wire Wire Line
	3600 950  3600 800 
Connection ~ 3600 800 
Wire Wire Line
	3600 800  3350 800 
$Comp
L Connector_Generic:Conn_01x02 J5
U 1 1 5B43FEE6
P 5150 4300
F 0 "J5" H 5229 4292 50  0000 L CNN
F 1 "5V" H 5229 4201 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-2POS" H 5150 4300 50  0001 C CNN
F 3 "~" H 5150 4300 50  0001 C CNN
	1    5150 4300
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x02 J6
U 1 1 5B440078
P 5600 4300
F 0 "J6" H 5679 4292 50  0000 L CNN
F 1 "5V" H 5679 4201 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-2POS" H 5600 4300 50  0001 C CNN
F 3 "~" H 5600 4300 50  0001 C CNN
	1    5600 4300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR09
U 1 1 5B440288
P 5300 4550
F 0 "#PWR09" H 5300 4300 50  0001 C CNN
F 1 "GND" H 5305 4377 50  0000 C CNN
F 2 "" H 5300 4550 50  0001 C CNN
F 3 "" H 5300 4550 50  0001 C CNN
	1    5300 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 4400 4950 4550
Wire Wire Line
	4950 4550 5300 4550
Wire Wire Line
	5400 4400 5400 4550
Wire Wire Line
	5400 4550 5300 4550
Connection ~ 5300 4550
Wire Wire Line
	5400 4300 5400 4150
Connection ~ 5400 4150
Wire Wire Line
	5400 4150 5850 4150
Wire Wire Line
	4950 4300 4950 4150
Connection ~ 4950 4150
Wire Wire Line
	4950 4150 5400 4150
Text Label 1050 3900 0    50   ~ 0
BTN_IN
Text Label 1050 4000 0    50   ~ 0
Z_PROBE_MOD
Text Label 1050 4500 0    50   ~ 0
~LCD_CS
Text Label 1050 5350 0    50   ~ 0
LCD_A0
Text Label 1050 5250 0    50   ~ 0
~LCD_RESET
Wire Wire Line
	1750 3300 1000 3300
Wire Wire Line
	1750 3400 1000 3400
Text Label 1050 3300 0    50   ~ 0
ENC_A
Text Label 1050 3400 0    50   ~ 0
ENC_B
Wire Wire Line
	10000 4400 9400 4400
Wire Wire Line
	10000 4500 9400 4500
Wire Wire Line
	10000 4600 9400 4600
Text Label 9700 4400 2    50   ~ 0
BTN_IN
Text Label 9450 4500 0    50   ~ 0
ENC_A
Text Label 9450 4600 0    50   ~ 0
ENC_B
$Comp
L Graphic:Logo_Open_Hardware_Large LOGO1
U 1 1 5C473C23
P 8850 4300
F 0 "LOGO1" H 8850 4800 50  0001 C CNN
F 1 "Logo_Open_Hardware_Large" H 8850 3900 50  0001 C CNN
F 2 "Symbol:OSHW-Logo2_7.3x6mm_SilkScreen" H 8850 4300 50  0001 C CNN
F 3 "~" H 8850 4300 50  0001 C CNN
	1    8850 4300
	1    0    0    -1  
$EndComp
$Comp
L Device:Fuse F2
U 1 1 5C49EAB2
P 7900 3700
F 0 "F2" H 7960 3746 50  0000 L CNN
F 1 "15A" H 7960 3655 50  0000 L CNN
F 2 "PrntrBoardV1:2410_FuseFolder" V 7830 3700 50  0001 C CNN
F 3 "~" H 7900 3700 50  0001 C CNN
	1    7900 3700
	1    0    0    -1  
$EndComp
Connection ~ 7900 3950
Wire Wire Line
	7900 3550 7900 3500
Wire Wire Line
	7900 3500 7500 3500
Text Label 7650 3500 0    50   ~ 0
Vin
Wire Wire Line
	1850 950  1850 900 
Wire Wire Line
	1850 900  1800 900 
$Comp
L Device:Fuse F1
U 1 1 5C5249D8
P 2150 800
F 0 "F1" V 1953 800 50  0000 C CNN
F 1 "10A" V 2044 800 50  0000 C CNN
F 2 "PrntrBoardV1:2410_FuseFolder" V 2080 800 50  0001 C CNN
F 3 "~" H 2150 800 50  0001 C CNN
	1    2150 800 
	0    1    1    0   
$EndComp
$Comp
L PrntrBoardV1-rescue:Screw_Terminal_01x02-Connector_Specialized J1
U 1 1 5ACB9063
P 1600 800
F 0 "J1" H 1750 850 50  0000 C CNN
F 1 "12-24V" H 1600 950 50  0000 C CNN
F 2 "TerminalBlock_MetzConnect:TerminalBlock_MetzConnect_Type701_RT11L02HGLU_1x02_P6.35mm_Horizontal" H 1600 800 50  0001 C CNN
F 3 "~" H 1600 800 50  0001 C CNN
	1    1600 800 
	-1   0    0    -1  
$EndComp
Wire Wire Line
	7900 3850 7900 3950
Text Label 1850 800  0    50   ~ 0
Vin
Wire Wire Line
	1800 800  2000 800 
$Comp
L pspice:DIODE D23
U 1 1 5D2C83BB
P 2600 800
F 0 "D23" H 2600 600 50  0000 C CNN
F 1 "S10M" H 2600 974 50  0000 C CNN
F 2 "Diode_SMD:D_SMC" H 2600 800 50  0001 C CNN
F 3 "~" H 2600 800 50  0001 C CNN
	1    2600 800 
	1    0    0    -1  
$EndComp
Wire Wire Line
	2800 800  3000 800 
Connection ~ 3000 800 
Wire Wire Line
	2400 800  2300 800 
Wire Wire Line
	7350 2150 7800 2150
Wire Wire Line
	7350 2300 7800 2300
Text Label 7450 2150 0    50   ~ 0
UART_XYZ
Text Label 7475 2300 0    50   ~ 0
UART_EX
Text Label 950  6550 0    50   ~ 0
UART_XYZ
Wire Wire Line
	3250 5750 3950 5750
Text Label 3725 5750 0    50   ~ 0
Z_END
Text Label 3825 5250 0    50   ~ 0
UART_EX
Text Label 1075 6925 2    50   ~ 0
~TC1_CS
$Comp
L Jumper:Jumper_3_Bridged12 JP4
U 1 1 5B24639E
P 1075 7225
F 0 "JP4" V 1029 7292 50  0000 L CNN
F 1 "E0_TC1" V 1120 7292 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 1075 7225 50  0001 C CNN
F 3 "~" H 1075 7225 50  0001 C CNN
	1    1075 7225
	0    -1   -1   0   
$EndComp
Text Label 600  6650 3    50   ~ 0
E1_COOL_PWM
Wire Wire Line
	1225 7225 1400 7225
Wire Wire Line
	1400 7225 1400 6750
Wire Wire Line
	1400 6750 1750 6750
Wire Wire Line
	1075 7475 1075 7675
Wire Wire Line
	1075 7675 1850 7675
Wire Wire Line
	1075 6975 1075 6925
Wire Wire Line
	1075 6925 775  6925
Wire Wire Line
	750  6500 600  6500
Wire Wire Line
	600  6500 600  7225
Text Label 1150 6750 0    50   ~ 0
E0_COOL_TC1
Wire Wire Line
	1000 6750 1400 6750
Connection ~ 1400 6750
$Comp
L Device:R R12
U 1 1 5D3D273F
P 3625 5250
F 0 "R12" V 3625 5250 50  0000 C CNN
F 1 "1k" V 3575 5100 50  0000 C CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 3555 5250 50  0001 C CNN
F 3 "~" H 3625 5250 50  0001 C CNN
	1    3625 5250
	0    1    1    0   
$EndComp
Wire Wire Line
	3250 5250 3300 5250
Wire Wire Line
	3775 5250 3950 5250
$Comp
L Device:R R10
U 1 1 5D3D4135
P 1475 6550
F 0 "R10" V 1475 6550 50  0000 C CNN
F 1 "1k" V 1525 6375 50  0000 C CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 1405 6550 50  0001 C CNN
F 3 "~" H 1475 6550 50  0001 C CNN
	1    1475 6550
	0    1    1    0   
$EndComp
Wire Wire Line
	1750 6550 1650 6550
Wire Wire Line
	1325 6550 900  6550
$Comp
L Device:R R44
U 1 1 5D3DCFC1
P 3300 4925
F 0 "R44" V 3300 4850 50  0000 L CNN
F 1 "10k" H 3100 4950 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 3230 4925 50  0001 C CNN
F 3 "~" H 3300 4925 50  0001 C CNN
	1    3300 4925
	1    0    0    -1  
$EndComp
Wire Wire Line
	3300 5075 3300 5250
Connection ~ 3300 5250
Wire Wire Line
	3300 5250 3475 5250
Wire Wire Line
	3300 4775 3300 4700
Wire Wire Line
	3300 4700 3875 4700
Text Label 3650 4700 0    50   ~ 0
3.3V
$Comp
L Device:R R38
U 1 1 5D3DDD26
P 1650 6975
F 0 "R38" V 1650 6900 50  0000 L CNN
F 1 "10k" H 1450 7025 50  0000 L CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 1580 6975 50  0001 C CNN
F 3 "~" H 1650 6975 50  0001 C CNN
	1    1650 6975
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 6825 1650 6550
Connection ~ 1650 6550
Wire Wire Line
	1650 6550 1625 6550
Wire Wire Line
	1650 7125 1650 7175
Wire Wire Line
	1650 7175 1450 7175
Text Label 1450 7175 0    50   ~ 0
3.3V
$Comp
L Connector_Generic:Conn_01x04 J37
U 1 1 5D3EFAE7
P 4450 7100
F 0 "J37" H 4530 7092 50  0000 L CNN
F 1 "RGB" H 4530 7001 50  0000 L CNN
F 2 "kicad-molex-sl-footprints:Molex-SL-4POS" H 4450 7100 50  0001 C CNN
F 3 "~" H 4450 7100 50  0001 C CNN
	1    4450 7100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0150
U 1 1 5D3F0150
P 4175 7375
F 0 "#PWR0150" H 4175 7125 50  0001 C CNN
F 1 "GND" H 4180 7202 50  0000 C CNN
F 2 "" H 4175 7375 50  0001 C CNN
F 3 "" H 4175 7375 50  0001 C CNN
	1    4175 7375
	1    0    0    -1  
$EndComp
Wire Wire Line
	4250 7300 4175 7300
Wire Wire Line
	4175 7300 4175 7375
$Comp
L Device:R R46
U 1 1 5D3F107D
P 4050 7000
F 0 "R46" V 4050 7000 50  0000 C CNN
F 1 "470" V 3975 7000 50  0000 C CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 3980 7000 50  0001 C CNN
F 3 "~" H 4050 7000 50  0001 C CNN
	1    4050 7000
	0    1    1    0   
$EndComp
$Comp
L Device:R R66
U 1 1 5D3F1912
P 4050 7100
F 0 "R66" V 4050 7100 50  0000 C CNN
F 1 "470" V 4100 7275 50  0000 C CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 3980 7100 50  0001 C CNN
F 3 "~" H 4050 7100 50  0001 C CNN
	1    4050 7100
	0    1    1    0   
$EndComp
$Comp
L Device:R R67
U 1 1 5D3F19ED
P 4050 7200
F 0 "R67" V 4050 7200 50  0000 C CNN
F 1 "470" V 4150 7200 50  0000 C CNN
F 2 "PrntrBoardV1:R_0603_1608Metric" V 3980 7200 50  0001 C CNN
F 3 "~" H 4050 7200 50  0001 C CNN
	1    4050 7200
	0    1    1    0   
$EndComp
Wire Wire Line
	4200 7000 4250 7000
Wire Wire Line
	4200 7100 4250 7100
Wire Wire Line
	4200 7200 4250 7200
Wire Wire Line
	3900 7200 3550 7200
Wire Wire Line
	3900 7100 3550 7100
Wire Wire Line
	3900 7000 3550 7000
Text Label 3625 7000 0    50   ~ 0
RED
Text Label 3625 7100 0    50   ~ 0
GREEN
Text Label 3625 7200 0    50   ~ 0
BLUE
Wire Wire Line
	3250 5650 3950 5650
Text Label 3800 5650 0    50   ~ 0
RED
Wire Wire Line
	3250 6250 3950 6250
Text Label 3675 6250 0    50   ~ 0
GREEN
Wire Wire Line
	1750 6650 675  6650
Text Label 725  6650 0    50   ~ 0
BLUE
Wire Wire Line
	10000 4800 9400 4800
Text Label 9450 4800 0    50   ~ 0
MISO
$Comp
L Device:Fuse F3
U 1 1 5D5CE9E3
P 5750 1340
F 0 "F3" V 5553 1340 50  0000 C CNN
F 1 "10A" V 5644 1340 50  0000 C CNN
F 2 "PrntrBoardV1:2410_FuseFolder" V 5680 1340 50  0001 C CNN
F 3 "~" H 5750 1340 50  0001 C CNN
	1    5750 1340
	0    1    1    0   
$EndComp
Wire Wire Line
	5900 1340 6050 1340
Wire Wire Line
	5600 1340 5500 1340
Text Label 5578 1340 2    50   ~ 0
Vin
$EndSCHEMATC
